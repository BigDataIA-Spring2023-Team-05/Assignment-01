...
# %%
def get_nexrad_aws_link(year, month, day, station, filename):
    # Stations, Year, Day, Hour
    copy_source = {
        'Bucket': nexrad_source_bucket,
        'Key': f'{year}/{month}/{day}/{station}/{filename}'
    }

    target_bucket.copy(copy_source, str(year) + '/' + str(month) + '/' + str(day) + '/' + str(station) + '/' + str(filename))

    return f'https://damg7245-team-5.s3.amazonaws.com/{year}/{month}/{day}/{station}/{filename}', f'https://noaa-nexrad-level2.s3.amazonaws.com/{year}/{month}/{day}/{station}/{filename}'
